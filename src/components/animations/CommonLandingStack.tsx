...
const LandingStackContext = createContext<LandingStackContextValue | null>(
  null,
);
...
export default function CommonLandingStack({
  ...
  useEffect(() => {
    ...
    const runSetup = () => {
      ...
      const tagsByCard: Array<Element | null> = cards.map((c) =>
        c.querySelector(".demo-card__tags"),
      );
      ...
      const onStRefresh = () => {
        if (cancelled) return;
        const c = cardRefs.current.filter((el): el is HTMLElement =>
          Boolean(el),
        );
        ...
        if (c.length && descrSplits.length) {
          const tags: Array<Element | null> = c.map((el) =>
            el.querySelector(".demo-card__tags"),
          );
      ...
      requestAnimationFrame(() => {
        if (cancelled) return;
        ScrollTrigger.refresh();
        const c2 = cardRefs.current.filter((el): el is HTMLElement =>
          Boolean(el),
        );
        if (c2.length) {
...
export function CommonLandingStackCard({
  ...
  return cloneElement(el, {
    ref: mergeRefs(el.props.ref, ctx.registerCard(index)),
  });
}
...
export function CommonLandingStackWrapper({
  ...
  const el = children as ReactElement<{ ref?: Ref<HTMLElement> }>;
  return cloneElement(el, {
    ref: mergeRefs(el.props.ref, ctx.registerWrapper(index)),
  });
}
...
export function CommonLandingStackImage({
  ...
  const el = children as ReactElement<{ ref?: Ref<HTMLElement> }>;
  return cloneElement(el, {
    ref: mergeRefs(el.props.ref, ctx.registerImage(index)),
  });
}
