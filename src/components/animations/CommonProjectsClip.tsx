...
export default function CommonProjectsClip({
  ...
  useLayoutEffect(() => {
    const triggers = triggerRefs.current.filter((el): el is HTMLElement =>
      Boolean(el && el.isConnected),
    );
    const items = itemRefs.current.filter((el): el is HTMLElement =>
      Boolean(el && el.isConnected),
    );
  ...
  return (
    <ClipContext.Provider
      value={{ registerTrigger, registerItem, registerBackground }}
    >
